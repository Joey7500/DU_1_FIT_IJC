...
inline void bitarray_setbit(bitarray_t jmeno_pole, bitarray_index_t index, int bool_vyraz) {
#ifndef NO_CHECK
...
inline int bitarray_getbit(bitarray_t jmeno_pole, bitarray_index_t index) {
#ifndef NO_CHECK
    if (index >= bitarray_size(jmeno_pole)) {
    ...
    return (jmeno_pole[(index / (sizeof(unsigned long) * CHAR_BIT)) + 1] & 
